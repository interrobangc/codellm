...
export const description: ToolDescription = {
  ...
  description: `This tool processes a query generated by an LLM to gather relevant code and code summaries from
    a vector db collection to provide context to assist the asking LLM in answering a question. It can summarize
    code and provide relevant code to the asker so that it can answer a question. The query should only include
    relevant functions, classes, types, and concepts that are relevant to the user's question.
  `,
  taskPrompt,
  params: [
    {
